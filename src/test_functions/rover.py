...
class RoverDomain:
    ...
    def __init__(
        ...
        if self.rnd_stream is None:
            ...
            self.rnd_stream = np.random.RandomState(123)
...
def create_large_domain(
    force_start=False, force_goal=False, start_miss_cost=None, goal_miss_cost=None
):
...
class Rover(TestFunction):
    def __init__(self, force_start: bool = False, force_goal: bool = False):
        ...
        self.d = self.domain.input_size
        self.bounds = np.repeat(self.domain.s_range, self.domain.traj.npoints, axis=1)
    ...
    def __call__(self, x: Normalized[Array, "d"]) -> Standardized[Array, ""]:
        ...
        try:
            ...
            print(f"Error evaluating cost at x={x}, with bounds={self.domain.s_range}")
            raise e
        return y
