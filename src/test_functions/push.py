import numpy as np
import pygame
from Box2D import b2World, b2PolygonShape, b2CircleShape, b2Vec2

# from Box2D.b2 import b2World
...
def run_simulation(
    ...
    rvel = b2Vec2(desired_vel[0], desired_vel[1])
    ...
    rvel2 = b2Vec2(desired_vel2[0], desired_vel2[1])
...
class Push(TestFunction):
    ...
    def __call__(self, x: Float[Array, "d"]) -> Float[Array, ""]:
        ...
        try:
            ...
            print(
                f"Error evaluating cost at x={x}, with bounds={self.push.xmin} and {self.push.xmax}"
            )
            raise e
        return y
